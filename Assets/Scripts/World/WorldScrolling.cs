...
public class WorldScrolling : MonoBehaviour
{
    ...
    public void UpdateTilesOnScreen()
    {
        for (int pov_x = -(fieldOfVisionWidth / 2); pov_x <= fieldOfVisionWidth / 2; pov_x++)
        {
            for (int pov_y = -(fieldOfVisionHeight / 2); pov_y <= fieldOfVisionHeight / 2; pov_y++)
            {
                ...
                //Debug.Log("Tile To Update_X " + tileToUpdate_x + " Tile To Update_Y " + tileToUpdate_y);
    ...
    public int CalculatePositionOnAxis(float currentValue, bool horizontal)
    {
        ...
        //else
        //{
        //    onTileGridPlayerPosition.y = terrainTileVerticalCount - 1 + playerTilePosition.y % terrainTileVerticalCount;
        //}
    }
}
